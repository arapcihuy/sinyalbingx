...
def fix_btc():
    ...
    data = res.get("data", [])
    if not data:
        print("❌ Tidak ada posisi open untuk BTC.")
        return
    ...
    print(f"✅ Posisi ditemukan: {side} {amt} BTC (Entry: {entry})")

    # 2. Hitung TP/SL Aman (Scalping 1% / 1.5%)
    if side == "LONG":
    ...
    # 3. Batalkan order lama BTC & pasang baru
